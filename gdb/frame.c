...
frame_find_by_id (struct frame_id id)
{
  ...
  if (cngdb_focus_on_device () && cngdb_on_start ())
...
get_prev_frame_always_1 (struct frame_info *this_frame)
{
  ...
  if (this_frame->level > 0
    ...
    {
      // Stop unwinding to cpu
      if (cngdb_focus_on_device() && cngdb_frame_outmost(this_frame)) 
      {
      ...
      {
        /** change pc_regnum to dwarf ra column for cngdb*/
        pc_regnum = cngdb_get_ra_dwarf_column (gdbarch);
      }

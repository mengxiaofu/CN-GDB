...
int always_device_breakpoint = 0;

extern int cngdb_auto_load;
                    ...
                    int *hw_bp_error_explained_already)
{
  ...
  if (bl->owner->cngdb_breakpoint)
    {
      bl->target_info.cngdb_breakpoint_location = 1;
    }
  else
    {
      bl->target_info.cngdb_breakpoint_location = 0;
    }
  ...
  ALL_BP_LOCATIONS (bl, blp_tmp)
    {
      ...
      if (cngdb_focus_on_device () ^ bl->owner->cngdb_breakpoint)
        continue;
                  ...
                  struct breakpoint *owner)
{
  ...
  switch (owner->type)
    ...
    case bp_dprintf:
    /* CNGDB - breakpoint */
    case bp_cngdb_driver_api_error:
    case bp_cngdb_driver_internal_error:
    case bp_cngdb_uvm:
                          ...
                          struct frame_id frame_id, enum bptype type)
{
  ...
  b->disposition = disp_donttouch;
  b->frame_id = frame_id;
  if (always_device_breakpoint ||
      (sal.symtab != NULL &&
      sal.symtab->compunit_symtab != NULL &&
      !strncmp (sal.symtab->compunit_symtab->producer, "Cambricon", 9)) ||
      cngdb_focus_on_device ())
    {
      b->cngdb_breakpoint = true;
    }
                     ...
                     int display_canonical)
{
      ...
      if (i == 0)
        {
          ...
          if (always_device_breakpoint ||
              (loc->symtab != NULL &&
              loc->symtab->compunit_symtab != NULL &&
              !strncmp (loc->symtab->compunit_symtab->producer, "Cambricon", 9)))
            {
              b->cngdb_breakpoint = true;
            }
        }
  ...
  struct breakpoint *ori;
  ALL_BREAKPOINTS (ori)
    {
      if (ori->cngdb_breakpoint == true)
        {
          if (ori->loc->address == b->loc->address - 1 ||
              ori->loc->address == b->loc->address + 1)
           {
             error (_("Set breakpoint failed, bad location"));
           }
        }
    }
...
update_global_location_list (enum ugll_insert_mode insert_mode)
{
  ...
  /* CNGDB - breakpoint */
  qsort (old_location, old_location_count, sizeof(*bp_location), bp_location_compare);
...
/* cngdb mark all breakpoints on device not inserted when kernel launch */
void
cngdb_mark_all_breakpoint_not_inserted (void)
{
  struct bp_location *bl, **blp_tmp;
  ALL_BP_LOCATIONS (bl, blp_tmp)
    {
      bl->inserted &= (!bl->owner->cngdb_breakpoint);
    }
}

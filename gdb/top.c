...
#define DEFAULT_PROMPT        "(cn-gdb) "
#endif

/* cngdb dir remove support */
...
print_gdb_version (struct ui_file *stream)
{
  ...
  fprintf_filtered (stream, "%s (based on GNU gdb %s)\n", PKGVERSION, version);
...
quit_force (char *args, int from_tty)
{
  ...
  sprintf (gdb_pid_dir_path, "%s%d", "/tmp/.cngdb/", getpid ());
  cngdb_remove_dir (gdb_pid_dir_path);

...
skip_prologue_sal (struct symtab_and_line *sal)
{
  ...
  /* cngdb : when sym != NULL & producer = Cambricon, don's do more anslysis */
  if (sym != NULL)
    {
      fixup_symbol_section (sym, NULL);
  ...
  /* cngdb hake here, because device obj and host obj in same elf file,
     so we need to judge gdbarch by pc again */
  struct compunit_symtab *cust = find_pc_compunit_symtab (pc);
  if (cust && !strncmp (cust->producer, "Cambricon", 9))
    {
      gdbarch = cngdb_get_gdbarch ();
    }

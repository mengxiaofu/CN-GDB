...
struct gdbarch
{
  ...
  int float16_bit;
  const struct floatformat ** float16_format;
  int double_bit;
               ...
               struct gdbarch_tdep *tdep)
{
  ...
  gdbarch->float16_bit = 2*TARGET_CHAR_BIT;
...
int
gdbarch_float16_bit (struct gdbarch *gdbarch)
{
  gdb_assert (gdbarch != NULL);
  /* Skip verify of float_bit, invalid_p == 0 */
  if (gdbarch_debug >= 2)
    fprintf_unfiltered (gdb_stdlog, "gdbarch_float16_bit called\n");
  return gdbarch->float16_bit;
}

void
set_gdbarch_float16_bit (struct gdbarch *gdbarch,
                         int float16_bit)
{
  gdbarch->float16_bit = float16_bit;
}
...
const struct floatformat **
gdbarch_float16_format (struct gdbarch *gdbarch)
{
  gdb_assert (gdbarch != NULL);
  if (gdbarch_debug >= 2)
    fprintf_unfiltered (gdb_stdlog, "gdbarch_float16_format called\n");
  return gdbarch->float16_format;
}

void
set_gdbarch_float16_format (struct gdbarch *gdbarch,
                          const struct floatformat ** float_format)
{
  gdbarch->float16_format = float_format;
}
...
target_gdbarch (void)
{
  /* cngdb hack here, when on device, return cambricon gdbarch */
